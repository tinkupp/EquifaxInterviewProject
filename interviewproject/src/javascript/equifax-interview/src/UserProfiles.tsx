...
const UserProfiles: React.FC = () => {
    ...
    const [formData, setFormData] = useState<User>({id: '', username: '', email: '', ssn: '' });
    ...
    const fetchUsers = async () => {
        ...
        const data: User[] = await res.json(); // data is an array of User objects without ssn field

        // Map to your local User type, with empty ssn (or leave undefined)
        const mappedUsers: User[] = data.map((user) => ({
            id: user.id,
            username: user.username,
            email: user.email,
            ssn: '', // ssn is not returned by the API here, so default empty string
        }));
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        if (!isEditing && formData.ssn) {
            // Only include SSN when creating (not updating)
